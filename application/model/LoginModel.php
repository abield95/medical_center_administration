<?php 
	...
	/**
	 * LoginModel
	 *The login part of the model: Handles the login / logout
	 *Need to prepare to use digital certificates
	*/
	class LoginModel
	{
		...
		/**
		 *Login process (DEFAULT user account=>hospital staff non administrators)
		 *@param $user_name string The user's name
		 *@param $user_password string The User's password
		 *@param $set_remember_me_cookie mixed Marker for usage of remember-me cookie features
		 *@return bool success state
		*/
		public static function login($user_name, $user_password, $set_remember_me_cookie = null)
		{

			//first we check, for empty username or empty password
			if (empty($user_name) OR empty($user_password)) {
				Session::add('feedback_negative', Text::get('FEEDBACK_USERNAME_OR_PASSWORD_FIELD_EMPTY'));
				return false;
			}

			//check if thne users exists, is login is not blocked (due to failed login) and if password fits the hash
			$result = self::validateAndGetUser($user_name, $user_password);

			//check if the user exists. We don't give back a cause in the feedback to avoid giving an atacker details
			if (!$result) {
				return false;
			}
			...
			//reset the failed login counter for that user (if necesary)
			...
			//if user has checked the remember me checkbox, the n write into database and into cookie
			if ($set_remember_me_cookie) {
				self::setRememberMeInDatabaseAndCookie($result->user_id);
			}

			//successfull logged in, so we write all encesary data into the session and set "user_logged_in" to true
		...
		private static function validateAndGetUser($user_name, $user_password)
		{
			...
			//get all data of that user (to later check if password and password_has fit)
			...
			if (!$result) {
				...
				Session::add(
						'feedback_negative',
						Text::get('FEEDBACK_USERNAME_OR_PASSWORD_WRONG')
					);
				return false;
			}

			//block login attemp if someone has failed 3 times and the last login attempt is less than 30sec ago
			...
			if (!password_verify($user_password, $result->user_password_hash)) {
				self::incrementFailedCounterOfuser($result->user_name);
				Session::add(
						'feedback_negative',
						Text::get('FEEDBACK_USERNAME_OR_PASSWORD_WRONG')
					);
			...
			if ($result->user_active != 1) {
				Session::add(
					'feedback_negative',
					Text::get('FEEDBAC_ACCOUNT_NOT_ACTIVATED_YET')
				);
				return false;
			}
